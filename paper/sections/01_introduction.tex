...
session context available at that instant --- raises the reachable share from 3\% to \textbf{90\%} on one
trace and 2\% to \textbf{53\%} on the other. What captures it is not prediction. First-come-first-served
admission with a fixed action reaches \textbf{93\% and 90\%} of the ceiling; a learned arrival-time
ranker at AUC 0.574 and 0.635 reaches 91.6\% and 88.9\%, slightly \emph{less}; and shuffling the arrival
order costs under three points. Almost all of the recoverable benefit is in the timing of the decision,
not in its content. The design implication is concrete and was invisible under the old framing: build an
\textbf{admission point}, not a predictor.
...
measure random eviction because a training buffer was truncated to its tail. In every case the aggregate
looked reasonable, which is the point --- none of our checks asked whether a reported quantity was
arithmetically able to be what it claimed.
...
\begin{itemize}
  ...
  share from 3\% to 90\% and 2\% to 53\%; there, first-come-first-served admission captures 93\% and 90\%
  of what is reachable and a learned ranker captures slightly less (\Cref{sec:arrival}). Once the metric
  is repaired, ranking quality converts monotonically but never enough to help
  (\Cref{sec:capacity}).
  ...
  \item \textbf{(C6)} A reusable evaluation protocol: ten measurement defects that each manufactured a
  plausible result here, grouped by mechanism, and the cheap controls that catch them --- including the
  rule that a null is reportable only alongside the sample size that would have resolved it
  (\Cref{sec:protocol}).
\end{itemize}
...
\input{sections/table1_positioning}
