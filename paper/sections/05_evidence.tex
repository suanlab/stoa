...
studies use --- ours for three revisions --- makes 95\% of the achievable benefit unreachable, so a
...
of a future in which they exist. Those blocks carry \textbf{94.7\% and 95.9\%} of the oracle's total
...
full hindsight gap & 93{,}383 & 94{,}714 \\
\textbf{attainable gap} (clairvoyance on visible blocks) & \textbf{4{,}996} & \textbf{3{,}900} \\
share of the gap a causal policy cannot reach & \textbf{94.7\%} & \textbf{95.9\%} \\
\bottomrule
\end{tabular}
\end{center}

\medskip
\noindent\textbf{It is not an artifact of where we cut.} Sweeping the split instant from 0.3 to 0.7
of the trace leaves the unreachable share at \textbf{91.8--98.4\%} on conversation and
\textbf{93.0--98.8\%} on toolagent --- and it is \emph{worse} at earlier splits, where more of the
trace is still to come. The pathology is a property of the protocol, not of our choice within it.

\medskip
\noindent A policy confined to 5\% of the prize will look flat no matter how well it ranks, and that is
...
\midrule
conversation & arrival-learned  & 0.574 & 70.5\% & 84.5\% \\
             & \textbf{arrival-FCFS} & 0.500 & \textbf{73.0\%} & \textbf{87.4\%} \\
             & arrival-shuffled & 0.500 & 70.2\% & 84.1\% \\
             & ceiling          & 1.000 & 83.5\% & 100.0\% \\
\midrule
toolagent    & arrival-learned  & 0.635 & 9.0\% & 47.2\% \\
             & \textbf{arrival-FCFS} & 0.500 & \textbf{9.6\%} & \textbf{50.2\%} \\
             & arrival-shuffled & 0.500 & 6.9\% & 36.1\% \\
             & ceiling          & 1.000 & 19.0\% & 100.0\% \\
\bottomrule
\end{tabular}
\end{center}

\medskip
\noindent Three readings survive and one does not.

\emph{Timing recovers much of the benefit.} The reachable share rises from 5.3\% to \textbf{83.5\%} on
conversation and 4.1\% to \textbf{19.0\%} on toolagent --- $16\times$ and $4.6\times$. The headroom was not
absent; the protocol could not reach it. The two traces differ by a factor of four in how much arrival
timing exposes, which we cannot explain from two exemplars.

\emph{Deciding at all captures most of it on one trace and half on the other.}
First-come-first-served admission with a fixed action reaches \textbf{87.4\%} of the ceiling on
conversation but only \textbf{50.2\%} on toolagent. An earlier draft of this paper reported 93\% and
90\% and concluded that ``deciding at all'' was sufficient; those numbers were computed before a
tie-breaking defect in the baseline routines was fixed, and the toolagent figure did not survive the
correction (\Cref{sec:protocol}).

\emph{Learning on top adds nothing.} The learned arm, at AUC 0.574 and 0.635, reaches 84.5\% and 47.2\%
--- below FCFS on both traces. This is the one claim that has survived every correction to the metric.

\emph{But the arrival \textbf{order} is not free, and we previously said it was.} Shuffling the order in
which arrivals are placed costs 3.3 points on conversation and \textbf{14.1 points} on toolagent. An
earlier draft reported ``under three points'' on both and concluded that almost all of the benefit was
in \emph{when} the decision is taken rather than in any ordering. On toolagent that is false:
first-come-first-served is doing real work there, and a deployment cannot treat arrival order as
incidental. We retract the claim.
...
\midrule
constant        & 0.50 / 0.50 & $-46.1$  & $-60.6$ \\
random          & 0.49 / 0.50 & $-208.2$ & $-838.6$ \\
$\log$(count)   & 0.64 / 0.71 & $-70.7$  & $-59.1$ \\
logistic        & 0.81 / 0.93 & $-18.8$  & $-32.1$ \\
GBDT            & 0.80 / 0.91 & $-23.9$  & $-32.8$ \\
...
\medskip
\noindent On toolagent the damage falls monotonically as AUC rises ($-60.6 \to -59.1 \to -32.1 \to
-19.3$), so ranking quality does convert --- into less harm. On conversation it does not: the constant
belief ($-46.1$) beats $\log$(count) ($-70.7$) despite carrying no information. An earlier draft claimed
monotonicity on both traces from numbers computed before the baseline tie-break was fixed; with the fix
it holds on one trace only, and we no longer claim it in general. Every arm is nonetheless below the
frequency baseline, because forcing an imperfect ranking onto the oracle's magnitude multiset assigns
large beliefs to blocks that will not be reused.
...
because they generalize, and because the later ones were found only after the earlier catalogue had been
written --- two by adversarial review, and three by re-verification passes run over a paper that had
already passed every check it defined for itself.
...
normalized against what is attainable, ranking quality converts monotonically on conversation and
non-monotonically on toolagent, so the repaired metric does not simply invert the retracted one. \textbf{(iii)} A reported
...
Two defensible repairs of that bug disagree by up to 2.1 points, and an independent reimplementation
produced during review differs from ours by 17 (\Cref{sec:reaction}). We therefore report the column as
a band and state the conclusion --- LRB below ARC at all eight operating points --- only at the
coarseness that band supports. An earlier draft of this paragraph compressed the two figures into a
single ``19 points'' attributed to the repairs, which is neither quantity.

Two further sub-claims were falsified in a later re-verification pass, after the paper had been
rewritten around the arrival result: that first-come-first-served admission captures roughly 90\% of
what is attainable (it captures 50.2\% on toolagent) and that the arrival \emph{order} is worth under
three points (14.1 on toolagent). Both had been stated from artifacts generated before a tie-break
fix to the routines that produce the denominator.
...
\paragraph{Derived and parallel surfaces outliving their source (three).} A result is not one file but a chain:
source $\rightarrow$ artifact $\rightarrow$ figure $\rightarrow$ paper. Fixing a link does not refresh
what follows it, and nothing in our pipeline compared adjacent links. Three artifacts the paper cited
were generated \emph{before} a fix to the code that produced them --- two of them denominators --- which
is how the two falsified sub-claims above reached a draft. After adding a freshness check on that first
edge and observing it pass, we found the plotted figures were fifteen days older than the artifacts they
draw: the text carried the corrected ladder while the figure still showed the retracted one, because the
paper embeds the PDF and a number-checker cannot see inside it. A freshness check on one edge reads like
a freshness check on the chain; it is not, and a guard that has only ever passed has not been tested.
The same claim also lives on surfaces that are not downstream of the paper but parallel to it --- a
repository README, a lab notebook --- and correcting the document that \emph{makes} a claim does not
correct the documents that \emph{recorded} it. Our notebook stated a falsified conclusion in the present
tense a hundred and fifty lines above its own retraction of it. Which repair applies depends on what the
surface is: an assertion is updated, a record is banner-marked and left intact.

\paragraph{Summaries of checked tables, unchecked (one).} Our checker re-derives every number in this
paper from its artifact. It verified all eight rows of the LRB band above and passed, while a sentence
elsewhere in the paper described that same band as ``two repairs disagreeing by 19 points'' --- the
repairs disagree by 2.1, and 17 is the gap to an independent reimplementation. Every element of the
table was right and the summary of it was false. Row-level verification certifies a table, not the
prose that aggregates it; widths, spans, maxima and ``at most'' claims need their own assertions,
derived from the same artifact. The deeper form of the same gap: every check we had asked whether the
\emph{correct} value was present, none whether a \emph{retracted} one was absent. Putting two
withdrawn figures back into our abstract leaves all of them passing, because the live values are still
in place further down.

\paragraph{Unverified interventions read as evidence (one).} Three times we concluded something from an
intervention that never took effect: a banner deleted only in part, a paragraph edited around a wrong
number, a mutation whose target string was absent and which therefore reported ``no test caught this''
--- indistinguishable from a real coverage gap. A negative result from an intervention you did not
confirm landed is not a result. Our harnesses now assert the change applied before reading the outcome,
and the guards are themselves tested in both directions: four had lived in a script where a refactor
could have deleted any one silently. A test's \emph{name} is a claim about coverage that nothing
checks --- ours named for the three-routine tie invariant compared two of them --- and the only way to
learn what a test pins is to break the thing and watch it fail --- having first checked that the
break changed the computation and not merely the file.

\paragraph{Emphasis without verification (one).} Our checker re-derives every number it is told
about, and for the life of this paper it was not told about the one the abstract leads with. The
``fraction of the oracle's advantage that is unreachable'' --- quoted in the abstract, in C2, in
\Cref{sec:reachable} and here --- sat in no artifact and in no assertion, and when finally recomputed
it was built from two different references: its attainable gap measured against the frequency
heuristic, its full gap against something 1.72$\times$ larger on both traces. The qualitative finding
survived; the figure moved from 97\% to 95\% and the timing multiplier from 27$\times$ to 16$\times$.
The guard is a coverage test rather than a correctness one: extract every emphasised number in the
source and fail if nothing asserts it. It answers ``what is nothing watching?'', which is a different
question from ``is what we watch correct?'' and the one we had never asked.
...
Sweeping three attention shapes against five device classes puts headroom at \textbf{67--90\%}, which
contains the 80\% our hand-written table gives --- the conclusion survives, but two things about the
