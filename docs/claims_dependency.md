...
## §AM — the figure guard was aimed at four figures the paper does not contain

Pass 11. Standard three steps clean (223 tests, 70/70, body 12 pages). §AD guarded figure
*freshness*; nothing had ever asked what the guarded figures were, or whether the paper used them.

The paper embeds two figures: `fig_reactive.pdf` and `fig_architecture.pdf`. `FIG_SOURCES` listed
five, of which **one** — `fig_reactive.pdf` — is embedded. The other four are generated by
`make_figures.py` and appear in no `.tex` file. `fig_architecture.pdf`, which the paper does embed,
was not in the list at all.

So §AD's claim to have closed the artifact → figure edge was one-fifth true. The guard fired
correctly on the one figure that mattered, which is why nothing looked wrong.

`paper/README.md` was worse: *"Two are used — `fig_reactive.pdf` and `fig_capacity_ladder.pdf`."*
`git log -S` finds `fig_capacity_ladder.pdf}` in no revision of any `.tex` file — it has **never**
been in this paper, nor in the preserved CIDR version. And in pass 2 I edited that sentence, changing
the figure's description from "the decoupling across model class" to "ranking quality against
captured benefit", to keep it consistent with the retraction. I revised the description of a figure
the paper does not contain, and did not check the sentence's premise while rewriting its predicate.

`verify.figure_coverage_gaps` now derives coverage from the `\includegraphics` calls themselves:
every embedded figure must have a freshness rule or be declared `data_free` (a schematic that plots
nothing), and every rule must point at an embedded figure. Both directions fail the run. `FIG_SOURCES`
is narrowed to the one figure it protects; the other six remain regenerable and unguarded, which is
the honest description of a plot the paper does not show.

**The generalization**: a guard's *target list* is as capable of being wrong as the thing it guards,
and a list that is mostly wrong still passes, because rules aimed at nothing never fire. Coverage
should be derived from the artifact being protected — here, the `.tex` — not maintained beside it.
Passes 7, 8 and 10 found tests whose names overstated what they pinned; this is the same error one
